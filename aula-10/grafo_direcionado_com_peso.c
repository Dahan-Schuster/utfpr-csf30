...
NoA* enqueue(NoA *lista, int id, int peso) {
...
void inserir_arestas_adj (GrafoA *G, int origem, int destino, int peso) {
   if (origem < 0 || origem >= G->V || destino < 0 || destino >= G->V) {
      printf("erro: um ou mais vértices inexistentes");
      return;
   }
   
   G->Adj[origem] = enqueue(G->Adj[origem], destino, peso);
}

void inserir_arestas_mat (GrafoM *G, int origem, int destino, int peso) {
   if (origem < 0 || origem >= G->V || destino < 0 || destino >= G->V) {
      printf("erro: um ou mais vértices inexistentes");
      return;
   }
...
int main () {
   ...
   inserir_arestas_adj(Ga, 0, 1, 1);
   inserir_arestas_adj(Ga, 0, 3, 5);
   inserir_arestas_adj(Ga, 1, 3, 8);
   inserir_arestas_adj(Ga, 2, 0, 5);
   inserir_arestas_adj(Ga, 2, 3, 4);
   inserir_arestas_adj(Ga, 3, 3, 6);
   inserir_arestas_adj(Ga, 3, 4, 3);
   inserir_arestas_adj(Ga, 4, 2, 7);
   inserir_arestas_adj(Ga, 4, 5, 9);
   inserir_arestas_adj(Ga, 5, 0, 2);
   ...
   // Matriz
   inserir_arestas_mat(Gm, 0, 1, 1);
   inserir_arestas_mat(Gm, 0, 3, 5);
   inserir_arestas_mat(Gm, 1, 3, 8);
   inserir_arestas_mat(Gm, 2, 0, 5);
   inserir_arestas_mat(Gm, 2, 3, 4);
   inserir_arestas_mat(Gm, 3, 3, 6);
   inserir_arestas_mat(Gm, 3, 4, 3);
   inserir_arestas_mat(Gm, 4, 2, 7);
   inserir_arestas_mat(Gm, 4, 5, 9);
   inserir_arestas_mat(Gm, 5, 0, 2);
